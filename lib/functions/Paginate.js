...
module.exports = class EasyEmbedPages {
    ...
    filter(interaction) {
        if (interaction.customId.match(new RegExp(`[1-5]-${this.user}-${this.id}`)))
            return true;
        return false;
    }
    ...
    generatePages() {
        ...
        for (let index = 0; index < x; index++) {
            ...
            if (this.dataPages[index] && this.dataPages[index].fields)
                this.dataPages[index].fields.map((x) => data.fields.push({ name: x.name || '\u200b', value: x.value || '\u200b', inline: x.inline || false }));
